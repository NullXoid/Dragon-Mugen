...
int runShopRoomMovementCollision(RuntimeProbe&, std::ostream& out) {
    ...
    const std::string runtimeText = readTextFile(std::filesystem::path(".") / "engine" / "src" / "ShopDemoRuntime.h");
    ...
    record(out, counts,
        ...
        "counter x=" + std::to_string(kShopVerifyCounterVisualCenterX) + " covers the customer-side service lane");
    record(out, counts,
    ...
    const size_t interactionStart = runtimeText.find("ShopInteractionKind shopDemoInteractionKind");
    const size_t talkReturn = runtimeText.find("return ShopInteractionKind::ShopkeeperTalk;", interactionStart);
    const size_t serviceReturn = runtimeText.find("return ShopInteractionKind::CounterService;", interactionStart);
    record(out, counts,
        interactionStart != std::string::npos
            && talkReturn != std::string::npos
            && serviceReturn != std::string::npos
            && talkReturn < serviceReturn
            ? Status::Pass : Status::Fail,
