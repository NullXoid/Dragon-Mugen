...
std::vector<OptionsMenuRowView> buildGameplayRows(const ControlsOptionsContext& context, int selected) {
    return {
        ...
        row("P2 PROFILE", compact(context.playerProfileNames[1], 18), selected == 2, true),
        row("FALL FALLBACKS", context.settings.fallFallbacksEnabled ? "ON" : "OFF", selected == 3, true),
        row("BACK", "", selected == 4),
    };
}
...
std::vector<OptionsMenuRowView> buildControlsRows(const ControlsOptionsContext& context, int selected) {
    return {
        row("PLAYER 1 CONTROLS", compact(context.playerProfileNames[0], 18), selected == 0),
        row("PLAYER 2 CONTROLS", compact(context.playerProfileNames[1], 18), selected == 1),
        row("PLAYER 3 CONTROLS", compact(context.playerProfileNames[2], 18), selected == 2),
        row("PLAYER 4 CONTROLS", compact(context.playerProfileNames[3], 18), selected == 3),
        row("KEYBOARD SETUP", "KEYS", selected == 4),
        row("CONTROLLER SETUP", context.padSummary, selected == 5),
...
std::vector<OptionsMenuRowView> buildPlayerRows(const ControlsOptionsContext& context, int selected) {
    ...
    rows.push_back(row("RESTORE PLAYER DEFAULTS", missingSummary(profile), selected == restoreRow));
...
std::vector<OptionsMenuRowView> buildKeyboardRows(const ControlsOptionsContext& context, int selected) {
    return {
        row("PLAYER 1 KEYBOARD", "ARROWS A/S/D Z/X/C", selected == 0),
        row("PLAYER 2 KEYBOARD", "IJKL U/O/P N/M/,", selected == 1),
        row("PLAYER 3 KEYBOARD", "TFGH Y/7/8 B/V/6", selected == 2),
        row("PLAYER 4 KEYBOARD", "NUMPAD", selected == 3),
        row("RESTORE KEYBOARD DEFAULTS", "ALL PLAYERS", selected == 4),
        row("BACK", "", selected == 5),
    };
}
...
std::vector<OptionsMenuRowView> buildInputTestRows(const ControlsOptionsContext& context, int selected) {
    return {
        row("PRESS INPUT", context.settings.controlStatusMessage.empty() ? "WAITING" : context.settings.controlStatusMessage, selected == 0, false),
        row("BACK", "", selected == 1),
    };
}
...
std::string optionsFooterText(const MainSettings& settings) {
    ...
    if (settings.optionsScreen == OptionsMenuScreen::PlayerControls) {
        return "UP/DOWN SEL  ENTER BIND  LEFT/RIGHT CHANGE  ESC BACK";
    }
    return "UP/DOWN SEL  LEFT/RIGHT CHANGE  ENTER SELECT  ESC BACK";
}
