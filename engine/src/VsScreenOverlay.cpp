...
float loadingVirtualWidth(const UiRenderContext& ui) {
...
void drawLoadingBackdrop(SDL_Renderer* renderer, float width, float height) {
    ...
    fillRect(renderer, 0.0f, 0.0f, width, 31.0f);
    setColor(renderer, 12, 22, 34, 235);
    fillRect(renderer, 0.0f, 31.0f, width, 68.0f);
    setColor(renderer, 5, 8, 13, 230);
    fillRect(renderer, 0.0f, 99.0f, width, height - 99.0f);

    setColor(renderer, 81, 210, 198, 42);
    fillRect(renderer, 18.0f, 70.0f, width - 36.0f, 1.0f);
    setColor(renderer, 231, 195, 90, 36);
    fillRect(renderer, 34.0f, 141.0f, width - 68.0f, 1.0f);
    setColor(renderer, 198, 79, 85, 220);
    fillRect(renderer, 0.0f, 28.0f, width, 2.0f);

    setColor(renderer, 12, 18, 26, 165);
    fillRect(renderer, width * 0.08f, 118.0f, width * 0.22f, 46.0f);
    fillRect(renderer, width * 0.70f, 112.0f, width * 0.20f, 54.0f);
}
...
void drawVersusScreenOverlay(const UiRenderContext& ui, const VsScreenView& view) {
    ...
    const float virtualWidth = loadingVirtualWidth(ui);
