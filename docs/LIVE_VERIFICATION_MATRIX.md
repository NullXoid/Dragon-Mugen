...
- Pause menu extraction checkpoint: render-only pause menu extraction built and scripted-verifier checked after build; file-size guard count for `engine/src/App.cpp` changed from `16330` to `16288`. Live match-pause GUI smoke is blocked by Computer Use being unable to move the SDL main-menu selector into Single Player/VS.
...
| Pause menu extraction | N/A | Single Player / VS | Reach match pause menu through current GUI controls | Enter a match mode and open the existing pause menu without changing behavior | Blocked: Computer Use Enter/Escape works, but Down/click did not move the SDL main-menu selector out of Training | BLOCKED | Do not count this as a runtime regression. Static extraction, build, dev checks, and scripted verifiers passed; manual match-pause GUI verification is still needed. |
| Pause menu extraction | Kung Fu Man | Training | Post-extraction Training smoke | Training path and overlays remain unaffected | Title -> Training -> character select -> stage select -> VS -> fight worked; command/input HUD, F1 hitboxes, F2 options, `R`, and Escape backout were observed | PASS | GUI smoke after pause extraction. This verifies unaffected Training render path, not match-pause rendering. |
