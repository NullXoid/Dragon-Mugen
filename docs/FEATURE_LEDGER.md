...
| `[VERIFIED-LIVE]` | Physical keyboard P1 input | Manual physical keyboard testing confirms KFM held walk, crouch, jump, standing normals, crouching normals, hit/damage, reset, F1/F2, Escape routing, run forward, and dash backward. P2 and controller input remain separate unverified areas. | Input runtime feature spec is replaced. |
...
| `[PARTIAL]` | CMD routing | Commands feed a buffered command system and State -1 routing rather than hardcoded character buttons. KFM scripted verification live-proved normal attack routing to states `210` and `410`; manual physical keyboard testing also confirmed KFM standing and crouching normals. Full special/super, P2, and controller command routing remain unverified. | Command runtime feature spec is replaced. |
...
| `[PARTIAL]` | Hit/guard/fall basics | KFM scripted verification live-proved `HitDef` contact, damage, hitpause, and hit sound on a standing normal. Guard, fall/recovery, guard spark/sound, KO, and broader character compatibility remain unverified. | Hit runtime feature spec is replaced. |
