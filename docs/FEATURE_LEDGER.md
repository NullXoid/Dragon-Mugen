...
| `[VERIFIED-STATIC]` | Pause menu rendering extraction | Render-only match pause menu label/layout drawing was extracted from `engine/src/App.cpp` into `engine/src/PauseMenuOverlay.h`. The extraction moved no pause input handling, selected-option mutation, resume/restart actions, routing, match reset, or fight runtime behavior. Build, dev checks, file-size guard, and scripted verifiers passed afterward. Live pause-menu GUI smoke remains blocked by the current automation inability to move the SDL main-menu selector into Single Player/VS. | A later module split replaces the transitional internal-header extraction. |
