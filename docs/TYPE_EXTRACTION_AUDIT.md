...
| `TrainingDebugOverlay.h` / `TrainingDebugOverlay.cpp` | F1 hitbox/debug overlay rendering | Normal module | Pass 40 converted render-only debug drawing to prepared `TrainingDebugView` data. Runtime projection remains in App.cpp-internal `TrainingDebugViewAssembly.h`. |
| `TrainingDebugViewAssembly.h` | F1 debug view assembly and screen-space projection | Transitional App.cpp internal | Depends on App.cpp-local fighter/stage/animation/camera/debug clipboard data. Keep internal until a fighter/debug projection boundary exists. |
| `TrainingCommandOverlay.h` / `TrainingCommandOverlay.cpp` | Training command/input HUD rendering | Normal module | Pass 38 converted render-only command/input HUD drawing to prepared `TrainingCommandHudView` data. Command matching and input history ownership remain outside the overlay. |
| `TrainingOptionsOverlay.h` / `TrainingOptionsOverlay.cpp` | F2 Training Options and move-list rendering | Normal module | Pass 36 converted render-only options/move-list drawing to prepared views. Pure option labels/status/toggle behavior lives in `TrainingOptionsBehavior.h/.cpp`; runtime side effects remain outside the overlay. |
...
| `UiRenderContext.h` / `UiRenderPrimitives.h` / `UiRenderPrimitives.cpp` | shared UI render primitives and scale/text helpers | Normal module | Pass 23 moved AppState-free color/rect drawing, SDL debug text, scaled debug text, fit text, panel helpers, and scoped UI scale into a normal module using resource-free `UiRenderContext`. App.cpp keeps only thin `AppState` bridge helpers. |
...
| Already extracted render overlays | menu/select overlays, fight HUD/result overlays, training options/command/debug overlays | normal modules plus remaining App.cpp-internal assembly bridges | `SAFE PURE DATA` to `MODERATE DATA` | Audit remaining transitional helpers before another conversion | HIGH | Front-end, fight, and training overlays now render through prepared view data. `UiRenderHelpers.h`, `FightPresentationShared.h`, `TrainingDebugViewAssembly.h`, `FrontendFlow.h`, and `AppVerificationBridge.h` remain transitional. |
...
| UI primitive/render helper surface | `setColor`, `scaledDebugText`, `drawPanel`, `ScopedUiScale`, title/select backgrounds, fixed opponent slot | 188 App.cpp reduction in Pass 17; Pass 23 added 13 App.cpp bridge lines and a 128-line normal primitive seam | `RESOURCE-COUPLED` | Primitive seam completed; sprite/title/select helpers still transitional | HIGH | AppState-free primitives now live in normal `UiRenderPrimitives.h/.cpp` via `UiRenderContext`. `UiRenderHelpers.h`, sprite helpers, texture creation, resource ownership, actor/fighter/stage rendering, and runtime rendering stayed in App.cpp/transitional headers. |
| Remaining transitional helper audit | `TRANSITIONAL_HELPER_BOUNDARY_AUDIT.md` | docs-only | `SAFE PURE DATA` to `BEHAVIOR-COUPLED` | Completed Pass 41 docs audit | HIGH | Recommends retiring `FightPresentationShared.h` first. `UiRenderHelpers.h` needs a public background/sprite presentation seam; `FrontendFlow.h`, `TrainingDebugViewAssembly.h`, and `AppVerificationBridge.h` remain deferred behind action/runtime seams. |
...
| Proposed Pass | Target | Estimated App.cpp Reduction | Risk | Preconditions | Test Focus |
| --- | --- | ---: | --- | --- | --- |
| Pass D0 | Completed Pass 23 public UI render primitive seam | +13 App.cpp bridge lines, removes primitive hidden coupling | LOW | `UiRenderContext`, normal primitive module | build/verifiers passed; no manual GUI smoke claimed |
| Pass D1 | Convert `PauseMenuOverlay.h` to a normal `.cpp` module | 0 physical App.cpp lines, removes hidden coupling | LOW | Public primitive seam and small `PauseMenuView` | pause render/resume plus Training route smoke |
...
| Item | Estimate |
| --- | ---: |
| Current `App.cpp` count | 14503 after Pass 23 |
| Known extracted headers/modules | 26 extracted headers/modules, including normal `TrainingState.h` / `FightMessageState.h` / `FightDisplayState.h` / `UiRenderContext.h` / `UiRenderPrimitives.h/.cpp` and transitional `FrontendFlow.h` / UI render helper headers |
