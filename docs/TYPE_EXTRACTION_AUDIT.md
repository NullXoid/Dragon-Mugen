...
| `TrainingState.h` | Training Options state storage | Normal header | Pass 19 moved only `TrainingOptions options` storage under `AppState::training`. `lastHitText` and `lastHitTextTicks` remain in App.cpp for a later fight-message/display state boundary. |
...
| fight/message display state | `lastHitText`, `lastHitTextTicks`, possible round/result callout display fields | 20-60 | `FightMessageState.h` or `FightDisplayState.h` | MEDIUM | Candidate future split after confirming the fields are display-only and do not drive hit/damage or round-decision behavior. |
...
| Pass C1a | Split `FightMessageState` / `FightDisplayState` for shared hit/result message fields | 20-60 | MEDIUM | Confirm `lastHitText` / `lastHitTextTicks` are display-only and not round-decision behavior | Training hit feedback, HUD display, result callouts, verifiers |
...
| Item | Estimate |
| --- | ---: |
| Current `App.cpp` count | 14495 after Pass 19 |
| Known extracted headers/modules | 22 extracted headers/modules, including normal `TrainingState.h` and transitional `FrontendFlow.h` / UI render helper headers |
