...
- Main menu `OPTIONS` currently exposes match timer length, output resolution, Dragon UI scale, controller label style, and P1/P2 gamepad assignment. Dragon defaults to fullscreen with a standard `1280x720 HD 720P` output preset and a `1280x720` windowed fallback. Output resolution presets are not alternate game-layout canvases: gameplay, menus, fight camera, and Dragon-owned UI stay on the fixed virtual presentation grid while the selected preset describes output resolution/quality for presentation and diagnostics. UI scale changes Dragon floating panel and callout density for readability only. This is Dragon-only runtime settings UI until equivalent M.U.G.E.N-compatible settings are parsed from `game/data` and local preferences are persisted in `game/save/settings.def`.
