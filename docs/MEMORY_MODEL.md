...
- Simple CNS `ChangeAnim` controllers can require `movecontact` plus `AnimElemTime(...)` comparisons and can jump to a target animation element. This covers KFM standing strong punch state `210` without hardcoding that state.
...
- Common dizzy actions `5300`/`5301` remain hittable in training and fight modes. If character CNS changes into a dizzy animation but its self-state exit does not fire in the current parser subset, Dragon releases it back to idle after the common dizzy window instead of leaving stale hit protection active.
