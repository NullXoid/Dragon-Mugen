...
The active code task is architecture recovery, tracked in [FEATURE_SPECS/0001_architecture_recovery.md](FEATURE_SPECS/0001_architecture_recovery.md). `App.cpp` growth is frozen by the architecture guard until owned modules are extracted. Do not add broad gameplay behavior as small app-layer patches. Feature work must follow [FEATURE_COMPLETION_POLICY.md](FEATURE_COMPLETION_POLICY.md): one active feature spec, source references, ownership, verification, and done criteria. KFM, Evil Ryu, Evil Ken, and future test characters must flow through `game/data/select.def` and the selected character DEF `[Files]` section after they are real filesets. Planned future work must be added to the appropriate roadmap/audit document and, when it represents content, to the matching M.U.G.E.N-style content folder so it cannot be lost as an untracked chat note. Do not start RPG/shop/equipment/tournament systems until KFM movement, one normal attack, dummy hitboxes, reset, and one working hit interaction are stable. The bgfx renderer remains required engine direction and should be addressed before broadening content or editor work.
