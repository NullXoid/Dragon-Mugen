...
## Compatibility Facade Baseline

Dragon now records a first-pass compatibility context for loaded content before runtime systems consume it.

- KFM resolves to `Mugen2001` from `mugenversion = 04,14,2001`.
- Evil Ryu and Evil Ken resolve to `Mugen10` and preserve `localcoord = 320,240`.
- Missing or unknown M.U.G.E.N versions default to `Mugen11`.
- The app shell defaults to `Dragon`, while character semantics still report M.U.G.E.N profiles unless Dragon content explicitly opts in.
- Dragon sidecars and legacy `[OpenBOR]` / `[DragonOpenBOR]` stage sections are exposed through compatibility metadata instead of becoming normal M.U.G.E.N file authority.
...
Future compatibility-sensitive fixes should use the facade as the decision point before changing throw binding, helpers/projectiles, hit/fall/stun, screen bounds, Arena depth, or input leniency.
...
The local Scott Pilgrim Versus package is used as an external stage compatibility proof only. `Tram_Rooftop` is mounted through git-ignored `game/data/external_content.local.def`, exercises IKEMEN-tolerant stage/select parsing, SFF v2 PNG/palette stage sprite decode, first-pass animated stage backgrounds, wide-viewport fixed-backdrop coverage, and MP3 stage BGM decode, but does not claim Scott character compatibility.
...
- IKEMEN `slot = { ... }` select metadata is ignored as metadata and must not create fake character or stage entries.
...
- SFF v2 PNG-backed stage sprites decode for stage archives, including indexed PNGs that rely on SFF v2 palette records. Character SFF v2 infrastructure exists, but full SFF v2 character compatibility is not claimed by this audit.
- Stage `[Music] bgmusic` paths decode WAV/MP3/OGG through SDL_mixer into Dragon's existing stage-music channel.
...
- Stage `[Bound] screenleft`/`screenright` are parsed and applied as default camera-visible player constraints using loaded character `[Size]` edge widths. Basic `ScreenBound` controllers are parsed for literal `value` and `movecamera`, so `value = 0` can opt a state out of the default screen clamp for one tick. Stage `[BGDef] spr` now resolves explicit SFF paths before falling back to same-stem SFF files, common duplicate `stages/` prefixes are tolerated, and first-pass `type = anim` / `actionno` BG elements render through `Begin Action` frames. Dragon also recognizes optional `[OpenBOR]` or `[DragonOpenBOR]` stage metadata for Arena-only side-scroller camera behavior; this metadata is a Dragon extension and does not change Training, Single Player, or VS camera behavior.
