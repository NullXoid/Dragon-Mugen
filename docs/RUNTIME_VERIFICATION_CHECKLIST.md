...
| CPU baseline scripted smoke | PASS | After the focused CPU bug fix, build, `dev_check`, `kfm-baseline`, `evilken-smoke`, `kfm-air-state`, and new `cpu-baseline` passed. `cpu-baseline` confirms Single Player fight phase, P2 movement toward P1, P2 attack state/action `200`, P1 still hitting/damaging P2, timer stability, and clean exit through the normal `FighterInputState`/CMD/CNS path. |
...
| Single Player CPU active fighting baseline | PASS | `--verify cpu-baseline` proves CPU-controlled P2 now feeds `FighterInputState` through `updateControlledFighter`, moves toward P1, attempts attack state/action `200`, can still be hit/damaged by P1, and keeps Single Player timer stability. This is a deterministic baseline, not advanced AI. |
