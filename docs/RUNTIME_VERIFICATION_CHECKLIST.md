...
| Post-TrainingState scripted checkpoint | PASS | After Training Options storage moved into normal header `engine/src/TrainingState.h`, build, `dev_check`, `kfm-baseline`, `evilken-smoke`, `kfm-air-state`, and `cpu-baseline` passed. This was storage-only; `lastHitText`/`lastHitTextTicks`, training runtime side effects, CPU behavior, routing, loading, and controller behavior stayed unchanged. No manual GUI/controller smoke is claimed for this pass. |
...
| Post-TrainingState storage split | PASS | `TrainingState.h` now stores `TrainingOptions options`, and all call sites use `state.training.options`. Scripted verifier coverage stayed green after the storage split. Manual F2 smoke was not rerun in this pass; prior Training Options live evidence and current scripted regression coverage remain the evidence. |
...
| Match-result controller D-pad navigation | PARTIAL | User reported controller confirm/cancel buttons worked on the `MATCH COMPLETE` screen but D-pad Up/Down did not move the result-menu highlight. `FrontendFlow.h` now maps match-result `SDL_GAMEPAD_BUTTON_DPAD_UP/DOWN` to the same `SDLK_UP/DOWN` path as keyboard navigation. Build/scripted regressions passed; physical controller retest is pending. |
